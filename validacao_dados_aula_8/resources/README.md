...
The databricks CLI bundles top level is defined by file `validacao_dados_aula_8/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`validacao_dados_aula_8/resources/model-workflow-resource.yml`)
 - batch inference workflow (`validacao_dados_aula_8/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`validacao_dados_aula_8/resources/monitoring-resource.yml`)
 - feature engineering workflow (`validacao_dados_aula_8/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`validacao_dados_aula_8/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `validacao_dados_aula_8/databricks.yml` and `validacao_dados_aula_8/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-validacao_dados_aula_8-model     | /dev-validacao_dados_aula_8-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-validacao_dados_aula_8-model | /staging-validacao_dados_aula_8-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-validacao_dados_aula_8-model    | /prod-validacao_dados_aula_8-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-validacao_dados_aula_8-model    | /test-validacao_dados_aula_8-experiment    |
...
2. Complete the `TODO` in `validacao_dados_aula_8/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `validacao_dados_aula_8/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `validacao_dados_aula_8/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-validacao_dados_aula_8-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-validacao_dados_aula_8-batch-inference-job`
that runs the notebook under `validacao_dados_aula_8/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-validacao_dados_aula_8-batch-inference-job`.
...
`test-validacao_dados_aula_8-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-validacao_dados_aula_8-batch-inference-job`.
